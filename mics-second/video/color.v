module color(
    ...
    wire [127:0] back;    // 背景（かご状の形）
    reg [6:0] block_pos;  // 動くブロックの位置（1次元）


    // 背景データ（かご状の形）

	assign back = 128'b0000000000000000 |
				 (128'b1000001000000001 << 16) |
				 (128'b1000001000000001 << 32) |
				 (128'b1000001000000001 << 48) |
				 (128'b1000001000000001 << 64) |
				 (128'b1000001000000001 << 80) |
				 (128'b1000001000000001 << 96) |
    ...
    always @(posedge Clk) begin
        if (Reset) begin
            ...
            if (move_enable) begin
                if (block_dir == 1 && block_pos > 7'd00) begin
                    // 左移動 (左端で止まる)
                    block_pos <= block_pos - 1;
                end else if (block_dir == 2 && block_pos < 7'd72) begin
                    // 右移動 (右端で止まる)
                    block_pos <= block_pos + 1;
                end
            end
    ...
    // 背景 + 動くブロックを統合
    always @(posedge Clk) begin
        map <= back | (1 << block_pos); // 背景とかご状ブロックをOR結合
    end
